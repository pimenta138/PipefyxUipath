...
{
    public class List_Pipes : CodeActivity
    {
        [Category("Input")]
        ...
        [Category("Input")]
        [Description("Pipe IDs to be listed")]
        [RequiredArgument]
        public InArgument<Array> PipeIDs { get; set; }
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            req.Method = "POST";
            byte[] bytes = System.Text.Encoding.UTF8.GetBytes("{ \"query\": \"{ pipes (ids: [" + PipeIDs.Get(context) + "]) { id name phases { name cards (first: 5) { edges { node { id title } } } } } }\" }");
